...
namespace Alloy.Sample.Business.Plugins
{
    [MenuProvider]
        ...
        public async Task<IActionResult> Generate([FromForm] GenerateContentPostDto dto)
        {
            ...
            for (var i = 0; i < dto.count; i++)
            {
                ...
                if (isPage)
                {
                    ...
                    if (dto.generateInlineBlocks)
                    {
                        var news = newContentItem as NewsPage;

                        var nestedContentArea = new ContentArea();

                        var nestedEditorialBlock = blockPropertyFactory.Create<EditorialBlock>();
                        nestedEditorialBlock.MainBody = new XhtmlString("Nested block = " + Guid.NewGuid());
                        var nestedContentAreaItem = new ContentAreaItem
                        {
                            InlineBlock = nestedEditorialBlock
                        };
                        nestedContentArea.Items.Add(nestedContentAreaItem);

                        var editorialBlock = blockPropertyFactory.Create<EditorialBlock>();
                        editorialBlock.MainBody = new XhtmlString(Guid.NewGuid().ToString());
                        editorialBlock.ContentArea = nestedContentArea;

                        var contentAreaItem = new ContentAreaItem
                        {
                            InlineBlock = editorialBlock
                        };

                        var newsMainContentArea = new ContentArea();
                        newsMainContentArea.Items.Add(contentAreaItem);
                        news.MainContentArea = newsMainContentArea;
                    }
                }
                else
                {
    ...
    public class GenerateContentPostDto
    {
        ...
        public bool generateInlineBlocks { get; set; }
